...
class GPT(nn.Module):
    ...
    @torch.no_grad()
    def generate_batched(self, idx, max_new_tokens, temperature=1.0, top_k=None, disable_pbar=False):
        ...
        # Cut the generated tensors to size
        B, M = idx.shape
        pattern_mask = (idx[:, :-1] == newline_id) & (idx[:, 1:] == newline_id)

        output = []
        for i in range(B):
            if pattern_mask[i].any():
                j = torch.where(pattern_mask[i])[0][0].item()
                output.append(idx[i, :j + 2])
            else:
                output.append(idx[i])
